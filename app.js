...
function openPlayerProfile(playerData) {
    ...
    const uLvl = document.getElementById('prof-level');
    const uBal = document.getElementById('prof-balance');
    const uVal = document.getElementById('prof-property-value');
    const uNet = document.getElementById('prof-networth');
    const uCnt = document.getElementById('prof-props-count');

    if (uName) uName.textContent = playerData.name;
    if (uLvl) uLvl.textContent = playerData.level;
    if (uBal) uBal.textContent = formatCurrency(playerData.balance);
    if (uVal) uVal.textContent = formatCurrency(playerData.propertyValue);
    if (uNet) uNet.textContent = formatCurrency(netWorth);
    if (uCnt) uCnt.textContent = playerData.properties.length;
