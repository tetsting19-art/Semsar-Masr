...
const MONTH_DAYS=30;
const YEAR_DAYS=360;
const initial={cash:START_CASH,rep:18,level:1,office:1,day:1,filter:"الكل",owned:[],stats:{deals:0,profit:0,flips:0},loans:[],rentOffers:[],notifications:[],employees:[]};
...
function cloneInitial(){return {cash:START_CASH,rep:18,level:1,office:1,day:1,filter:"الكل",owned:[],stats:{deals:0,profit:0,flips:0},loans:[],rentOffers:[],notifications:[],employees:[]}}
...
async function saveCloud(){
 ...
 const {error}=await sb.from("game_profiles").upsert({
  ...
  owned:state.owned,stats:state.stats,loans:state.loans,rent_offers:state.rentOffers,employees:state.employees,net_worth:Math.round(netWorth()),updated_at:new Date().toISOString()
 },{onConflict:"id"});
 if(error)console.error(error);
}
async function loadCloud(){
 const {data,error}=await sb.from("game_profiles").select("username,cash,rep,level,office,day,owned,stats,loans,rent_offers,employees").eq("id",user.id).maybeSingle();
 ...
 state={...cloneInitial(),cash:Number(data.cash),rep:Number(data.rep),level:Number(data.level),office:Number(data.office),day:Number(data.day),
  ...
  loans:Array.isArray(data.loans)?data.loans:[],rentOffers:Array.isArray(data.rent_offers)?data.rent_offers:[],employees:Array.isArray(data.employees)?data.employees:[]};
}
async function auth(){
 ...
 try{
  if(signup){
   ...
   const {error:pe}=await sb.from("game_profiles").insert({id:user.id,username,cash:START_CASH,rep:18,level:1,office:1,day:1,owned:[],stats:initial.stats,loans:[],rent_offers:[],employees:[],net_worth:START_CASH});
...
function render(){
 ...
 renderFilters();renderListings();renderPortfolio();renderCities();renderAchievements();renderBank();renderOffers();renderOffice();renderEmployeeSection();renderNotifications();renderActivity();
}
...
async function buy(id){
 ...
 state.stats.deals++;state.rep=clamp(state.rep+1,0,100);if(state.stats.deals%3===0)state.level++;
...
function counterRent(id){
 const o=state.rentOffers.find(x=>x.id===id);if
